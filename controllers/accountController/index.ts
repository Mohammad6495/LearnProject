...
import UserServices from "../../services/account_Services";
...
export const loginAction = async (
  ...
  try {
    ...
    const data = await services.LoginService({ password, userName });
    res.json({ ...data });
  } catch (err) {
    next(err);
  }
...
export const SignAction = async (
  ...
  try {
    ...
    const data = await services.SignUpService({ password, userName });
    res.json({ ...data });
  } catch (err) {
    next(err);
  }
